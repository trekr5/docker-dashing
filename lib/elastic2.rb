module ElasticQuery2
  ...
  def elastic_query_2(statname, timeframe, title, last, current)
            ...
            time_now, prev_day, prev_hour, prev_prev_day = prev_date_and_current_time
             ...
             client = Elasticsearch::Client.new hosts: [{host: 'view.logs.prod.aws.justgiving.service', port: 80 ,user: 'showme', password: 'thel0gs'}]
                                          ...
                                          "bool"=> {
                                            ...
                                            {"term"=> {"#{statname}"=> "donation"}},
                                             ...
                                             "range"=> {
                                              ...
                                              "should"=> [
                                                {"term" => {"#{statname}"=> "sponsorship"}},
                                                {"term" => {"#{statname}"=> "direct"}},
                                               # {"term" => {"#{statname}"=> "charity"}},
                                                   ]
                                       }
                                      }
                                    }
     ...
     value_current_midnight = client.search index: current_logstash_date, body: hash_midnight_now
       ...
       File.open(LOG_FILE, "a+"){ |f| f.puts " title: #{title}, value_current_awesome query failure #{Time.now} err=#{err}"}
       File.open(LOG_FILE, "a+"){ |f| f.puts " title: #{title}, value_current_awesome query failure #{Time.now} err=#{err.backtrace.join("\n")}"}
    end
...
File.open(LOG_FILE, "a+"){ |f| f.puts " title: #{title}, value_last_awesome query failure #{Time.now} err=#{err}"}
File.open(LOG_FILE, "a+"){ |f| f.puts " title: #{title}, value_last_awesome query failure #{Time.now} err=#{err.backtrace.join("\n")}"}
    end
